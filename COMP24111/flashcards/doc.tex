...
\card{
    Briefly explain the hierarchical clustering apporoach
}{
...
\flashcard{
    In order to calculate the F-ratio index we divide the \blank{intra-cluster} variance by the \blank{inter-cluster} variance.
}
...
\card{
    ...
    Run the clustering algorithm ranging from $K=2$ up to $n$. Then find the F-ratio index for each $K$ value, and use the value with the lowest F-ratio index.
}
\end{document}
