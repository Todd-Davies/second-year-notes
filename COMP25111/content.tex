...
It was aimed at home use, and concequently, early versions (upto and including
...
operating system that is compaible (with DOS emulation) with older windows
...
is built into the OS, and system calls are not publically accessible, but are
...
\begin{description}
  \item \textbf{P} - \textbf{Hardware Abstraction layer (HAL)}\\
    ...
    timing on the clock, interrupt handeling etc.
  ...
  \item \textbf{U} - \textbf{Environments}\\
    ...
    machines, OS/2 is a OS/2 1.x emuator for 16-bit character based
...
\begin{description}
  ...
  \item \textbf{Configuration manager}\\
    ...
    however, it's also a single point of faliure, and hard to edit, back up and
  ...
  \item \textbf{Kernel}\\
    ...
    fibers are scheduled by libraries run inside threads (so a fiber is like a
    psudo thread).
    ...
    priority, the foreground application recieves a larger time slice, and
  ...
  \item \textbf{IO}\\
    ...
    as USB). The device info oo will tell the IO manager where to find the
    driver for the device.

    Drivers use object based methods (such as \texttt{init}, \texttt{interrupt}
    etc) to interface with hardware. Direct Memory Access is avalible to
...
Concieved in 1969 as a single-user system, and expanded in 1973 to be a multi-
...
The philospohy behind unix is that there should be lots of very simple programs
